...
class SCIM20
{
	...
	private function parseUserPayload($payload, $userCheck=false)
	{
		...
		foreach($payload as $key => $value)
			if(!in_array($key,array('schemas', 'id', 'externalId', 'meta', 'userName', 'name', 'displayName', 'nickName', 'profileUrl', 'title', 'userType', 'preferredLanguage', 'locale', 'timezone', 'active', 'password', 'emails', 'phoneNumbers', 'ims', 'photos', 'addresses', 'groups', 'entitlements', 'roles', 'x509Certificates','oditlyRole')) && !in_array($key, $schemas))
